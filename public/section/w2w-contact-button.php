		...
		$html = '
			...
			<div id="w2w-widget-flyout" class="pos-' . $w2w_classes . ' w2w-pos-fixed w2w-pinkBg">
				...
				if(!empty(w2w_get_option( 'txtEmail' ))){
					$html .= '<li class="btn-email"><span class="w2w-icon"><i class="fas fa-envelope"></i></span><a href="mailto:' . w2w_get_option( 'txtEmail' ) . '" target="_blank"><span class="w2w-text">' . __( 'Email' , 'w2w' ). '</span></a>
					</li>';
				}
				if( !empty(w2w_get_option( 'gr-phone' ))) {
					foreach (w2w_get_option( 'gr-phone' ) as $key => $value){
						//echo w2w_get_option( 'gr-phone' )[$key]['txtName'] .' '. w2w_get_option( 'gr-phone' )[$key]['txtPhoneNumber'];
						$html .= '
							<li class="btn-phone">
								<span class="w2w-icon"><i class="fab fa-whatsapp"></i></span>
								<a href="tel:' . w2w_get_option( 'gr-phone' )[$key]['txtPhoneNumber'] . '">
									<span class="w2w-text">' . w2w_get_option( 'gr-phone' )[$key]['txtName'] . 
									((w2w_get_option( 'fs-style' )['opt-show'] == 'icontext')? ' ' . w2w_get_option( 'gr-phone' )[$key]['txtPhoneNumber']: '') . '</span>
								</a>
							</li>';
					}
				}
				...
				if(!empty(w2w_get_option( 'txtZalo' ))){
					$html .= '<li class="btn-zalo"><span class="w2w-icon"><span class="icon-zalo"></span></span><a href="//zalo.me/' . w2w_get_option( 'txtZalo' ) . '" target="_blank"><span class="w2w-text">Zalo' . ((w2w_get_option( 'fs-style' )['opt-show'] == 'icontext')? ' ' . w2w_get_option( 'txtZalo' ): '') . '</span></a>
					</li>';
				}
		$html .='
				</ul>
